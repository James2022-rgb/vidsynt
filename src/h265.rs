...
mod tests {
    ...
    fn read_nalus() {
...

        {
            ...
            let mut reader = LengthPrefixedByteStreamNaluReader::with_length_size_minus_one(
                3,
                reader,
                nalu_value_context,
            );
    ...
    fn read_nalu_refs_in_mp4() {
        ...
        let mp4_sample = mp4_reader.read_sample(TRACK_ID, 1).unwrap().unwrap();

        println!(
            "start_time: {}, duration: {}",
            mp4_sample.start_time, mp4_sample.duration
        );
