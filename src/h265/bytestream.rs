...
pub type LengthPrefixedByteStreamNaluReader<R> = LengthPrefixedByteStreamContentReader<R, NaluReader>;
pub type LengthPrefixedByteStreamNaluRefReader<R> = LengthPrefixedByteStreamContentReader<R, NaluRefReader>;

impl<R> LengthPrefixedByteStreamNaluReader<R> {
  pub fn with_length_size_minus_one(length_size_minus_one: usize, inner_reader: R, nalu_value_context: NaluValueContext) -> Self {
    Self {
      length_size_minus_one,
      inner_reader,
      content_reader: NaluReader {
        nalu_value_context,
      },
    }
  }
...
impl<R: Read> ReadContent<R, Nalu> for NaluReader {
  fn read_content(&mut self, reader: &mut R, length: usize, current_offset: usize) -> Result<(usize, Nalu), io::Error> {
...
impl<R: Read + Seek> ReadContent<R, NaluRef> for NaluRefReader {
  fn read_content(&mut self, reader: &mut R, length: usize, current_offset: usize) -> Result<(usize, NaluRef), io::Error> {
...
pub trait ReadContent<R: Read, T> {
  ...
  fn read_content(&mut self, reader: &mut R, length: usize, current_offset: usize) -> Result<(usize, T), io::Error>;
}
...
impl<R: Read + Seek, CR> LengthPrefixedByteStreamContentReader<R, CR> {
  ...
  pub fn read_content<T>(&mut self, current_offset: usize) -> Result<ByteStreamContent<T>, io::Error>
  where
    CR: ReadContent<R, T>,
  {
    ...
    let (content_consumed, content) = self.content_reader.read_content(&mut self.inner_reader, length, current_offset)?;
    ...
    if remaining > 0 {
      ...
      self.inner_reader.seek(io::SeekFrom::Current(remaining.try_into().unwrap()))?;
    }
  ...
  fn read_length(&mut self) -> Result<usize, io::Error> {
    let mut buf = [0; 4];
    self.inner_reader.read_exact(&mut buf[..=self.length_size_minus_one])?;
    let length = buf[..=self.length_size_minus_one].iter().fold(0, |acc, &x| acc << 8 | x as usize);
    Ok(length)
  }
}
...
pub fn convert_length_prefixed_to_annex_b<'a>(
  ...
  for nalu_content in nalu_contents.into_iter() {
    let extend_vec_with_bytes = |dst: &mut Vec<u8>, bytes: &[u8]| {
      ...
      unsafe {
        std::slice::from_raw_parts_mut(dst.as_mut_ptr().add(dst.len()), bytes.len()).copy_from_slice(bytes);
    ...
    let nal_unit_bytes = &length_prefixed_byte_stream[nalu_content.offset..nalu_content.offset + nal_unit_size];
