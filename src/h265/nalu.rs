...
impl Nalu {
    pub fn from_bytes(
        bytes: &[u8],
        nalue_value_context: NaluValueContext,
    ...
    pub fn from_ebsp_reader<R: Read>(
        ...
        let ebsp = {
            ...
            reader.read_exact(unsafe {
                std::slice::from_raw_parts_mut(ebsp.as_mut_ptr(), value_length)
            })?;
            // SAFETY:
        ...
        match nalu_header.nal_unit_type {
            ...
            NaluType::PpsNut => {
                let value = PictureParameterSet::from_rbsp_reader(
                    rbsp_reader,
                    nalu_header.nuh_temporal_id_plus1,
                )?;
