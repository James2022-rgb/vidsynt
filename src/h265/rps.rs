...
impl ShortTermReferencePictureSet {
  ...
  pub fn rps_idx_num_delta_pocs(&self) -> Option<u8> {
    match &self.value {
      ShortTermReferencePictureSetValue::InterRefPicSetPrediction(value) => value.rps_idx_num_delta_pocs,
      ShortTermReferencePictureSetValue::NonInterRefPicSetPrediction(_) => None,
    }
  }
  ...
  pub fn num_negative_pics(&self) -> u8 {
    match &self.value {
      ...
      ShortTermReferencePictureSetValue::NonInterRefPicSetPrediction(value) => value.num_negative_pics,
    }
  }
  pub fn num_positive_pics(&self) -> u8 {
    match &self.value {
      ShortTermReferencePictureSetValue::InterRefPicSetPrediction(_) => todo!(),
      ShortTermReferencePictureSetValue::NonInterRefPicSetPrediction(value) => value.num_positive_pics,
    }
  }
  ...
  pub fn delta_poc_s0_minus1(&self) -> [u16; 16] {
    match &self.value {
      ShortTermReferencePictureSetValue::InterRefPicSetPrediction(_) => todo!(),
      ShortTermReferencePictureSetValue::NonInterRefPicSetPrediction(value) => value.delta_poc_s0_minus1,
    }
  }
  pub fn used_by_curr_pic_s0_flag(&self) -> [bool; 16] {
    match &self.value {
      ShortTermReferencePictureSetValue::InterRefPicSetPrediction(_) => todo!(),
      ShortTermReferencePictureSetValue::NonInterRefPicSetPrediction(value) => value.used_by_curr_pic_s0_flag,
    }
  }
  pub fn delta_poc_s1_minus1(&self) -> [u16; 16] {
    match &self.value {
      ShortTermReferencePictureSetValue::InterRefPicSetPrediction(_) => todo!(),
      ShortTermReferencePictureSetValue::NonInterRefPicSetPrediction(value) => value.delta_poc_s1_minus1,
    }
  }
  pub fn used_by_curr_pic_s1_flag(&self) -> [bool; 16] {
    match &self.value {
      ShortTermReferencePictureSetValue::InterRefPicSetPrediction(_) => todo!(),
      ShortTermReferencePictureSetValue::NonInterRefPicSetPrediction(value) => value.used_by_curr_pic_s1_flag,
    }
  }

  pub fn bitmask_used_by_curr_pic_s0_flag(&self) -> u16 {
    // Convert the `used_by_curr_pic_s0_flag` array to a bitmask.
    self.used_by_curr_pic_s0_flag().iter().enumerate().fold(0, |acc, (i, &flag)| {
      acc | ((flag as u16) << i)
    })
  }
  ...
  pub fn from_bit_reader<R: Read>(
    ...
    Self::from_bit_reader_impl(bit_reader, st_rps_index, num_short_term_ref_pic_sets, None, bit_count)
  }
  ...
  fn from_bit_reader_impl<R: Read>(
    ...
    let value = if inter_ref_pic_set_prediction_flag.unwrap_or(false) {
      ...
      *bit_count += 1;
      ...
      let abs_delta_rps_minus1: u16 = read_exp_golomb_ue_count_bits(bit_reader, bit_count)? as _;

      let rps_idx_num_delta_pocs = if let Some(delta_idx_minus1) = delta_idx_minus1 {
        let slice_sps_st_ref_pic_sets = slice_sps_st_ref_pic_sets
          .expect("st_ref_pic_set() in slice header. slice_sps_st_ref_pic_sets must be Some");
      ...
      for i in 0..num_negative_pics {
        delta_poc_s0_minus1[i as usize] = read_exp_golomb_ue_count_bits(bit_reader, bit_count)? as _;
        *bit_count += 1;
      ...
      for i in 0..num_positive_pics {
        delta_poc_s1_minus1[i as usize] = read_exp_golomb_ue_count_bits(bit_reader, bit_count)? as _;
        *bit_count += 1;
        used_by_curr_pic_s1_flag[i as usize] = bit_reader.read_bit()?;
      }

      ShortTermReferencePictureSetValue::NonInterRefPicSetPrediction(NonInterRefPicSetPrediction {
        num_negative_pics,
        num_positive_pics,
        delta_poc_s0_minus1,
