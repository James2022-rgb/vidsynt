...
impl SequenceParameterSet {
  pub fn from_rbsp_reader<R: Read>(reader: &mut R) -> Result<Self, io::Error> {
    ...
    let log2_diff_max_min_luma_coding_block_size: u8 = read_exp_golomb_ue(&mut bit_reader)? as _;
    let log2_min_luma_transform_block_size_minus2: u8 = read_exp_golomb_ue(&mut bit_reader)? as _;
    let log2_diff_max_min_luma_transform_block_size: u8 = read_exp_golomb_ue(&mut bit_reader)? as _;
    ...
    let short_term_ref_pic_sets = {
      ...
      let mut short_term_ref_pic_sets: Vec<ShortTermReferencePictureSet> = Vec::with_capacity(num_short_term_ref_pic_sets as _);
...
impl Vui {
  pub fn from_bit_reader<R: Read>(bit_reader: &mut BitReader<R, BigEndian>) -> Result<Self, io::Error> {
