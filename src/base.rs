...
mod tests {
  ...
  fn de_exp_golomb_ue(skip: u8, seq: u32) -> u32 {
    ...
    bit_reader.seek_bits(io::SeekFrom::Current(skip as i64))
      .unwrap();

    let leading_zero_count = bit_reader.read_unary1()
      .unwrap();
    (1 << leading_zero_count) - 1 + bit_reader.read::<u32>(leading_zero_count)
      .unwrap()
  }
  ...
  fn en_exp_golomb_ue(code_num: u32) -> (u32, u32) {
    ...
    bit_writer.write(num_bits_code_num_plus1, code_num_plus1)
      .unwrap();
